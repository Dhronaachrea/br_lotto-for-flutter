  ...
  DateTime? pickedDate = await showDatePicker(
    ...
    builder: (context, child) {
      return Theme(
        data: Theme.of(context).copyWith(
          colorScheme: const ColorScheme.light(
            primary: BrLottoPosColor.orangey_red,
            onPrimary: BrLottoPosColor.white,
            onSurface: BrLottoPosColor.black,
          ),
          textButtonTheme: TextButtonThemeData(
            style: TextButton.styleFrom(
              primary: BrLottoPosColor.icon_green, // button text color
            ),
          ),
        ),
        child: child!,
...
String getLanguage() {
  if(SnackbarGlobal.key.currentContext != null){
    return BrLottoRetailApp.of(SnackbarGlobal.key.currentContext!).locale.languageCode == "fr" ? "pt": BrLottoRetailApp.of(SnackbarGlobal.key.currentContext!).locale.languageCode;
  } else {
    return "en";
  }
}
...
//For eg -> model.responseData?.data?[0].cURRENCYLIST -> "en#.#UAH#N~uk#,#UAH#N#,";
  ...
  var language = "en"; // ToDo remove it when it is fixed from backend
...
String getLanguageWithContext(BuildContext context){
  return BrLottoRetailApp.of(context).locale.languageCode == "fr" ? "pt": BrLottoRetailApp.of(context).locale.languageCode ;
}
