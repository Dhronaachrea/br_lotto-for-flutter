...
import 'package:br_lotto/scratch/packOrder/game_details_logic.dart';
import 'package:br_lotto/scratch/packOrder/model/PackOrderRequest.dart';
import 'package:br_lotto/scratch/packOrder/model/game_details_response.dart';
import 'package:br_lotto/scratch/packOrder/model/pack_order_response.dart';
import 'package:br_lotto/scratch/packOrder/pack_order_logic.dart';
import 'package:br_lotto/scratch/packReceive/book_receive_logic.dart';
import 'package:br_lotto/scratch/packReceive/dl_details_logic.dart';
import 'package:br_lotto/scratch/packReceive/model/book_receive_response.dart';
import 'package:br_lotto/scratch/packReceive/model/dl_details_request.dart';
import 'package:br_lotto/scratch/packReceive/model/dl_details_response.dart';
import 'package:br_lotto/scratch/pack_activation/game_list_logic.dart';
import 'package:br_lotto/scratch/pack_activation/model/game_list_request.dart';
import 'package:br_lotto/scratch/pack_activation/model/game_list_response.dart';
import 'package:br_lotto/scratch/pack_activation/model/pack_activation_response.dart';
import 'package:br_lotto/scratch/pack_activation/pack_activation_logic.dart';
import 'package:br_lotto/scratch/pack_return/model/pack_return_note_response.dart';
import 'package:br_lotto/scratch/pack_return/model/response/game_vise_inventory_response.dart';
import 'package:br_lotto/scratch/pack_return/model/response/pack_return_submit_response.dart';
import 'package:br_lotto/scratch/pack_return/pack_return_note_logic.dart';
  ...
  try {
    response.when(
        ...
        responseFailure: (value) {
          ...
          emit(PackError(errorMessage: loadLocalizedData("SCRATCH_${errorResponse.responseCode ?? ""}", BrLottoRetailApp.of(context).locale.languageCode) ?? errorResponse.responseMessage ?? ""));
        },
        failure: (value) {
...
_onGameListEvent(GameListApi event, Emitter<PackState> emit) async {
  ...
  try {
    response.when(
        ...
        responseFailure: (value) {
          print("bloc responseFailure:");
          GameListResponse errorResponse = value as GameListResponse;
          emit(PackError(errorMessage: loadLocalizedData("SCRATCH_${errorResponse.responseCode ?? ""}", BrLottoRetailApp.of(context).locale.languageCode) ?? errorResponse.responseMessage ?? ""));
        },
        failure: (value) {
          print("bloc failure: ${value["occurredErrorDescriptionMsg"]}");
...
_onPackActivationEvent(PackActivationApi event, Emitter<PackState> emit) async {
  ...
  try {
    response.when(
        ...
        responseFailure: (value) {
          print("bloc responseFailure:");
          PackActivationResponse errorResponse = value as PackActivationResponse;
          emit(PackError(errorMessage: loadLocalizedData("SCRATCH_${errorResponse.responseCode ?? ""}", BrLottoRetailApp.of(context).locale.languageCode) ?? errorResponse.responseMessage ?? ""));
        },
        failure: (value) {
          print("bloc failure: ${value["occurredErrorDescriptionMsg"]}");
